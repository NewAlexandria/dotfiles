...
1. **Symlinks dotfiles into `$HOME`** -- every top-level file not in the `DOTFILE_SKIPLIST` (see `Rakefile:4-18`) gets linked as `~/.dotfiles/<file>` -> `~/<dot><file>`. For example `zshrc` becomes `~/.zshrc`. The interactive prompt asks before overwriting existing files (answer `a` to replace all, `y`/`n` per file).
...
4. **On macOS**, runs `mac/setup.sh`, installs app configs (Karabiner, Velja), and sets up ZSH with oh-my-zsh + powerlevel10k.
...
~/.dotfiles/
  ...
  ackrc, agignore, ctags, gemrc, hgrc, irbrc, asdfrc, ...

  # Function libraries (sourced by zshrc/bash_profile)
...
`functions_shell.sh` sets `DOTFILES_REPO="/Users/$(whoami)/.dotfiles"`. This variable is available in all shells sourced from zshrc/bash_profile. The `cddot` alias navigates to it.

## Adding custom local/private configuration

The repo supports a **local override file** that is never committed:

- **`~/.zshrc_local.sh`** -- sourced at the end of `zshrc` (line 30-32) if it exists. Put machine-specific PATH additions, secrets, employer-specific aliases, or anything private here.
...
The `.gitignore` also excludes `ssh_config` and `.profile.local` from tracking.
...
| `rake mac:install_configs` | Symlink Karabiner, import Velja prefs |
...
- Top-level files without extensions are dotfile configs that get symlinked with a `.` prefix.
- Shell function files are organized by domain (`_shell`, `_dev`, `_osx`, `_colors`, `_graphics`).
- macOS app configs live under `mac/` and are installed via rake tasks, not symlinks (many macOS apps require `defaults import` or copies rather than symlinks).
